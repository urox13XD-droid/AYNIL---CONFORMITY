...
function mergeLayout(partial: Partial<ConformityLayoutMap> | undefined): ConformityLayoutMap {
  ...
  for (const key of ELEMENT_KEYS) {
    ...
    merged[key] = {
      pos: { ...DEFAULT_LAYOUT[key].pos, ...override.pos },
      style: { ...DEFAULT_LAYOUT[key].style, ...override.style },
    };
  }
  return merged;
}

const DRAFT_KEY = "aynil-conformity:draft";

export function loadDraft(): ConformitySheetData | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(DRAFT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<ConformitySheetData>;
    return {
      ...EMPTY_SHEET,
      ...parsed,
...
export function saveDraft(sheet: ConformitySheetData) {
